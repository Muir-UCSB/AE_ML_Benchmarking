...
'''Read-in'''
raw = '210330-1_waveforms.txt'
filter = '210330-1_filter.csv'
...
print(feat_vect_set)
'''Kmeans'''
kmeans = KMeans(n_clusters=k,max_iter=300,n_init=100).fit(feat_vect_set[3])
labels = kmeans.labels_

''' Tsne manipulation'''
tsne_data = feat_vect_set[3]

tsne = TSNE(n_components=2, verbose=1, perplexity=15, n_iter=2000, random_state=1) #Note:random_state to get reproducible results
tsne_results = tsne.fit_transform(tsne_data)

plt.figure(figsize=(6,6))
sns.scatterplot(x=tsne_results[:,0], y=tsne_results[:,1],
                hue = labels,
                palette=sns.color_palette('icefire',k),
                data=tsne_results,
                legend = 'full',
                style = labels,
                alpha=0.7)
